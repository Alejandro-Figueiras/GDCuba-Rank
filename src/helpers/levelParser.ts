...
export const parseDifficulty = (level: Level) => {
  const difficulty = level.difficultydenominator
    ? level.difficultynumerator / level.difficultydenominator
    : 'N/A'
  const featured =
    level.epic == 3
      ? 'Mythic'
  ...
  const difficultyName = level.auto
    ? 'Auto'
    : level.demon
      ? level.demondifficulty == 3
        ? 'Easy Demon'
        : level.demondifficulty == 4
          ? 'Medium Demon'
          : level.demondifficulty == 5
            ? 'Insane Demon'
            : level.demondifficulty == 6
              ? 'Extreme Demon'
              : 'Hard Demon'
      : difficulty == 0
        ? 'N/A'
  ...
  return {
    ...
    difficultyName,
    difficultyNumber: getDifficultyNumber({
      difficulty,
      demon: level.demon,
...
export const getDifficultyNumber = ({
  ...
  demondifficulty = 3,
  demon = false
}: DifficultyNumberParams) => {
  if (demon) {
    return demondifficulty == 3
      ? 11 // Easy Demon
      : demondifficulty == 4
        ? 12 // Medium Demon
        : demondifficulty == 5
