...
export const ResponsiveRankNav = ({onLinkSelected, info}) => {
    ...
    <Accordion aria-label={info.title} className="px-0">
      ...
      >
        {info.items.map(itemInfo => {
          ...
          return (
            <NavMenuLink href={itemInfo.href} fontSize={"text-lg"} key={itemInfo.key}>
              <div className={`flex gap-2 justify-start items-center mb-3 ${itemInfo.href == route && "text-cyan-600"}`} onClick={onLinkSelected}>
