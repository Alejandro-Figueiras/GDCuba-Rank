...
const getIcon = async ({
  ...
  if (username) {
    const gdacc = JSON.parse(await getAccountAction({ username }))
    if (gdacc) {
      switch (type) {
        case 'ship':
