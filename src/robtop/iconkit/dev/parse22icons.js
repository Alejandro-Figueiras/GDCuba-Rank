...
const logic = async () => {
  ...
  for (const icon of folder) {
    ...
    if (parts[1] == 'plist') {
      ...
      if (array[1] == 'uhd') {
        // Parse plist
        const archivo = plist.parse(
          fs.readFileSync('./icons/' + icon, 'utf8')
        ).frames
        const info = {}
        ...
        for (const frameName of Object.keys(archivo)) {
          ...
          for (const keyName of Object.keys(archivo[frameName])) {
            ...
            if (
              ['spriteOffset', 'spriteSize', 'spriteSourceSize'].includes(
                keyName
              )
            ) {
              ...
              let textureArr = keyData
                .slice(1, -1)
                .split('},{')
                .map((x) => parseWeirdArray(x))
...
const parsePlist = () => {
  ...
  for (const frameName of Object.keys(iconFrames)) {
    ...
    for (const keyName of Object.keys(frameData)) {
      ...
      if (
        ['spriteOffset', 'spriteSize', 'spriteSourceSize'].includes(keyName)
      ) {
